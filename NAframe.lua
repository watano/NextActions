...
function NA_Option_Frame_OnLoad(self)
	...
	local left = 5;
	--enable profile
	local frame0 = W_createNCBtn("cbtnNA_Profile",NA_MaxProfile,self,"P:", top, left);
	...
	top = top - frame0:GetHeight();
	local frame0 = W_createNCBtn("cbtnNA_Flag",3,self,"F:", top, left);
	getglobal("cbtnNA_Flag"..0):SetScript("OnClick", 
		function(self, button, down)
			NA_IsAOE = self:GetChecked() == 1;
		end
	);
	getglobal("cbtnNA_Flag"..1):SetScript("OnClick", 
		function(self, button, down)
			NA_IsMaxDps = self:GetChecked() == 1;
		end
	);
	getglobal("cbtnNA_Flag"..2):SetScript("OnClick", 
		function(self, button, down)
			NA_IsSolo = self:GetChecked() == 1;
		end
	);
	top = top - frame0:GetHeight();

	self:SetHeight(top*-1+6);
	self:SetWidth(95);
	self:Show();
end
...
function NA_frame_set_color_hex(fname, hex)
   if (fname and hex) then
      ...
      W_Log(1,"NA_frame_set_color_hex=>"..tonumber(rhex, 16) .."--".. tonumber(ghex, 16) .."--".. tonumber(bhex, 16));
      frame:SetBackdropColor(tonumber(rhex, 16)/255, tonumber(ghex, 16)/255, tonumber(bhex, 16)/255);
   end
end

function NA_ShowVars(v1)
   W_Log(1, "NA_ShowVars=>" ..v1 .."--"..NAParseValue2Color(v1));
   if(not NA_IsTest) then NA_frame_set_color_hex("NA_Main_Frame", NAParseValue2Color(v1)); end
end

function NAParseColor2Value(color)
   local rhex, ghex, bhex = string.sub(color, 1+2, 1+2), string.sub(color, 3+2, 3+2), string.sub(color, 5+2, 5+2);
   return tonumber("0x" .. rhex .. ghex .. bhex, 10);
end 

function NAParseValue2Color(value)
   local v = string.format("%03x", value);
   return "0x" .. string.sub(v,1,1) .. "7" .. string.sub(v,2,2) .. "7" .. string.sub(v,3,3) .. "7";
end
